...
import { RiLogoutCircleRLine, RiSearch2Line, RiSettingsLine } from "react-icons/ri";
  ...
  return (
    <div className="w-full h-fit py-6 px-[50px] bg-white flex justify-between">
      ...
      <div className="flex items-center gap-[25px]">
        <button
          className="text-xl text-secend"
        >
          <IoNotificationsOutline />
        </button>
        <div className="flex flex-col ">
          <h2 className="text-lg font-medium font-poppins text-secend">{reduxData.userName}</h2>
          <p className="text-sm font-bold text-secend">{reduxData.userRole}</p>
        </div>
        <button onClick={()=>set_show_profile_option(!show_profile_option)} className="w-[48px] h-[48px] bg-brandColor rounded-full">
        ...
        </button>
          {
            ...
            <div className= " bg-white  rounded-[5px]  absolute top-[10%] right-10 shadow-[1px_0px_11px_0px_rgba(0,_0,_0,_0.1)] text-center">
              ...
              <button onClick={handelLogOut} className="flex items-center  px-5 py-2 gap-3 hover:bg-brandColor hover:text-white duration-[.4s]"><RiLogoutCircleRLine />Logout</button>
            </div>
          }
      </div>
